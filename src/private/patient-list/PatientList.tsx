...
const PatientList = () => {
    ...
    try {
      ...
      const url =
        selectedButton === "Pending"
          ...
          : selectedButton === "Assigned" ? "http://ec2-3-110-204-139.ap-south-1.compute.amazonaws.com/api/AD/getAssignedPatient" : "http://ec2-3-110-204-139.ap-south-1.compute.amazonaws.com/api/AD/getCompletedQueue";
  ...
  const handleRejectAppointment = async (email: string) => {
    try {
      ...
      const response = await axios.get(
        `http://ec2-3-110-204-139.ap-south-1.compute.amazonaws.com/api/AD/rejectAppointment?email=${emailSent}`,
        {
  ...
  const handleCompleteAppointment = async (email: string) => {
    try {
      ...
      const emailSent = email.substring(0,email.indexOf("@"))+email.substring(email.indexOf("@")).replace(".",",");
      const response = await axios.get(
        `http://ec2-3-110-204-139.ap-south-1.compute.amazonaws.com/api/AD/completeAppointment/${emailSent}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleReassign = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://ec2-3-110-204-139.ap-south-1.compute.amazonaws.com/api/AD/reassign",
        {
