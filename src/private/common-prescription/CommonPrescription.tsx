  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const apiUrl =
          role === "doctor"
            ? `http://localhost:8080/api/doctor/getPrescription/${urlParam}`
            : role === "ad"
            ? `http://localhost:8080/api/AD/getPrescription/${urlParam}`
            : `http://localhost:8080/api/patient/getPrescription/${urlParam}`;
