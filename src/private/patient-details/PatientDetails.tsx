...
const PatientDetails = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const resp = await axios.post(
        "http://localhost:8080/api/doctor/prescription/submit",
        req,
        {
          headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const resp = await axios.get(
          "http://localhost:8080/api/doctor/getPatient",
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
    ...
    const fetchMed = async () => {
      ...
      try {
        const resp = await axios.get(
          `http://localhost:8080/api/doctor/stock/available`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
              "X-Latitude": latitude,
  ...
  const handleRelease = async () => {
    try {
      const resp = await axios.get(
        "http://localhost:8080/api/doctor/releasePatient",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        ...
        <div className="flex justify-between mb-4 max-lg:flex-col max-lg:items-center">
          <div className="flex justify-center items-center w-[25%] max-lg:w-[50%] max-lg:mb-4">
            <img
              ...
              src={
                ndata?.imageUrl != null
                  ? `http://localhost:8080/${ndata?.imageUrl}`
                  : "/default-user.jpg"
              }
            />
          </div>
